...
Eventos são ações ou ocorrências que acontecem durante a execução do robô, como a abertura de uma ordem, o encerramento de uma operação ou a validação de um gatilho. Sempre que uma tarefa relevante é realizada, o robô dispara um evento correspondente.

Esses eventos podem ser monitorados e utilizados para automatizar as ações do robô em diferentes situações de mercado. Assim, o usuário pode configurar regras e estratégias personalizadas que respondem automaticamente a ocorrências específicas, como o fechamento de uma posição com lucro, a execução de uma ordem ou a identificação de um sinal de compra.

Na prática, os eventos tornam o ScriptBot mais inteligente, dinâmico e flexível, permitindo a execução de ações específicas em tempo real. Isso resulta em maior controle e eficiência das estratégias.
